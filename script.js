...
// ==========================================
// SECURITY HARDENING: HTML SANITIZATION (ANTI-XSS)
// ==========================================
/**
 * Fungsi untuk menyaring input teks / variabel data dinamis dari tag HTML berbahaya.
 * Mencegah celah keamanan DOM-based Cross-Site Scripting (XSS).
 */
...
const ADMIN_CONFIG = {
  ...
  practiceQuestions: [
    ...
    },
    {
      ...
      options: [
        "Menpaga penyalahgunaan kekuasaan eksekutif.",
  ...
  tryoutQuestions: [
    ...
    },
    {
      id: 38,
      category: "Tes Inteligensia Umum (TIU)",
      question: "Lima orang anak perempuan: Ani, Ana, Dini, Rani, dan Lusi memiliki mainan boneka. Boneka milik Lusi lebih banyak dari milik Rani and Dini. Boneka milik Ana lebih banyak dari Dini, tapi tidak lebih banyak dari Lusi. Boneka Ani sama banyaknya dengan Dini. Boneka Rani lebih banyak dari Ani. Siapakah pemilik boneka paling banyak?",
      options: [
        "Ana",
        "Ani",
        "Dini",
    ...
    },
    {
      ...
      answer: 1,
      explanation: "Menokak gratifikasi secara halus dan tegas menjaga integritas diri sebagai auditor profesional tanpa menyinggung pihak lain."
    },
    {
      id: 71,
      category: "Tes Karakteristik Pribadi (TKP) - Profesionalisme",
    ...
    },
    {
      ...
      answer: 0,
      explanation: "Pemasahan masalah pribadi dengan profesionalitas kerja adalah kunci integritas dan kematangan emosi petugas pelayanan."
    },
    {
      id: 86,
    ...
    },
    {
      id: 91,
      category: "Tes Karakteristik Pribadi (TKP) - Jejaring Kerja",
      question: "Rahmad mengerjakan tugas tim dengan bersungguh-sungguh sehingga dinilai bagus oleh atasannya. Namun ternyata ada salah seorang teman di tim tersebut mengaku bahwa dialah yang mengerjamannya, padahal semua itu adalah kerja Rahmad sendiri. Sikap apa yang diambil jika Anda sebagai Rahmad?",
      options: [
    ...
    },
    {
      ...
      options: [
        ...
        "Menuju kesana untuk memperbaikinya and meminta teman untuk menyelesaikan pekerjaan yang sedang Saya kerjakan saat ini."
      ],
      answer: 3,
    ...
    },
    {
      ...
      options: [
        ...
        "Meminta maaf kepada manajer tersebut atas kurang akuratnya data yang dikirim, lalu menjanjikan laporan resmi dikirim sendiri oleh staf bersangkutan."
      ],
      answer: 2,
    ...
    },
    {
      ...
      answer: 3,
      explanation: "Membantu mencarikan solusi alternatif saat kapasitas finansial pribadi terbatas menunjukkan empati kemainan yang aktif dan cerdas."
    },
    {
      id: 103,
      category: "Tes Karakteristik Pribadi (TKP) - Sosial Budaya",
    ...
    },
    {
      ...
      options: [
        ...
        "Mbahas permasalahan yang saya hadapi dengan rekan kerja.",
...
function navigateTo(targetView, pushState = true) {
  ...
  // Update navbar visual indicator
...
// Menyorot Tab Menu Aktif
function updateActiveNav(targetView) {
  // Desktop
  const desktopBtns = document.querySelectorAll('#desktop-nav button');
  desktopBtns.forEach(btn => {
    btn.className = "nav-btn px-4 py-2 rounded-lg text-sm text-gray-300 hover:text-white hover:bg-maroon/20 transition-all";
    if (btn.getAttribute('onclick') && btn.getAttribute('onclick').includes(`'${targetView}'`)) {
      btn.className = "nav-btn px-4 py-2 rounded-lg text-sm text-gold bg-maroon/30 border border-maroon/50 font-bold transition-all";
    }
  });

  // Mobile
  const mobileBtns = document.querySelectorAll('#mobile-nav-links button');
  mobileBtns.forEach(btn => {
    btn.className = "w-full text-left px-4 py-3 text-stone-300 hover:text-white hover:bg-maroon/20 rounded-lg transition";
    if (btn.getAttribute('onclick') && btn.getAttribute('onclick').includes(`'${targetView}'`)) {
      btn.className = "w-full text-left px-4 py-3 text-gold bg-maroon/30 border-l-4 border-maroon font-bold rounded-lg transition";
    }
  });
}
...
// ==========================================
// ADMIN RENDER LOGIC WITH SANITIZATION GUARDS
// ==========================================
function renderPromoCodes() {
  ...
  ADMIN_CONFIG.promoCodes.forEach(promo => {
    ...
    // Menerapkan escapeHTML untuk memblokir eksploitasi tag HTML dinamis
    div.innerHTML = `
      <div>
        <span class="text-[10px] text-maroon-light bg-maroon/20 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider inline-block mb-3">${escapeHTML(promo.category)}</span>
        <p class="text-stone-400 text-xs mb-1">${escapeHTML(promo.desc)}</p>
      </div>
      <div class="mt-4 pt-3 border-t border-stone-800/60 flex items-center justify-between">
        <span class="text-sm font-bold tracking-wider font-mono text-gold">${escapeHTML(promo.code)}</span>
        <button onclick="copyToClipboard('${escapeHTML(promo.code)}')" class="text-xs bg-maroon hover:bg-maroon-light text-white px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 font-semibold">
  ...
  // Re-inisialisasi ikon Lucide setelah elemen dinamis dimuat ke DOM
...
function renderBooks() {
  ...
  const generateCardHtml = (book) => `
    <div class="bg-stone-950 border border-stone-800 rounded-3xl p-4 flex flex-col justify-between hover:border-maroon/60 transition-all shadow-xl group">
      <div class="relative overflow-hidden rounded-2xl bg-stone-900 mb-4 flex items-center justify-center p-6 h-64">
        <span class="absolute top-2 left-2 bg-maroon text-white font-bold text-[9px] px-2 py-0.5 rounded-full z-10">${escapeHTML(book.tag)}</span>
        <img src="${escapeHTML(book.image)}" alt="${escapeHTML(book.title)}" class="max-h-full max-w-full object-contain transition duration-500 group-hover:scale-105" onerror="this.onerror=null; this.src='https://placehold.co/300x400/800000/FFFFFF?text=Buku+Al+Faiz'">
      </div>
      <div class="space-y-3">
        <h3 class="font-bold text-white text-base leading-snug line-clamp-2 h-12">${escapeHTML(book.title)}</h3>
        <div class="flex items-center justify-between border-t border-stone-900 pt-3">
          <div>
            ...
            <span class="text-gold font-bold text-lg">${escapeHTML(book.price)}</span>
          </div>
          <a href="${escapeHTML(book.link)}" target="_blank" rel="noopener noreferrer" class="bg-maroon hover:bg-maroon-light text-white font-bold text-xs px-4 py-2.5 rounded-xl transition flex items-center gap-1">
  ...
  if (container) {
    ...
    ADMIN_CONFIG.books.forEach(book => {
      container.innerHTML += generateCardHtml(book);
    });
  }
  ...
  if (homeContainer) {
    ...
    ADMIN_CONFIG.books.forEach(book => {
      homeContainer.innerHTML += generateCardHtml(book);
    });
  }
...
function copyToClipboard(text) {
  ...
  // Memastikan isi clipboard bersih dari karakter injeksi berbahaya
  el.value = escapeHTML(text);
  document.body.appendChild(el);
  el.select();
  document.execCommand('copy');
  document.body.removeChild(el);
  showAlert(`Kode "${escapeHTML(text)}" berhasil disalin!`);
}

// User Authentication with Sanitization Guards
...
function handleAuthSubmit(e) {
  ...
  // Sanitasi & validasi input sebelum diproses oleh Auth API
    ...
    if (authMode === 'login') {
      auth.signInWithEmailAndPassword(emailInput, passInput)
        .then((userCredential) => {
          ...
          // VALIDASI STATUS: Apakah email sudah diverifikasi?
          if (user.emailVerified) {
            showAlert("Selamat datang kembali! Anda berhasil masuk.");
            closeAuthModal();
          } else {
            // Beri peringatan dan paksa logout jika belum verifikasi
            showAlert("Akun Anda belum aktif. Silakan verifikasi email Anda terlebih dahulu (cek kotak masuk atau folder spam).");
            auth.signOut();
          }
        })
        .catch(err => {
          showAlert(`Gagal Masuk: ${escapeHTML(err.message)}`);
        });
    } else {
      auth.createUserWithEmailAndPassword(emailInput, passInput)
        .then((userCredential) => {
          ...
          // LANGKAH AKTIVASI: Otomatis kirim email verifikasi setelah akun terbuat
          user.sendEmailVerification()
            .then(() => {
              showAlert("Registrasi sukses! Silakan periksa kotak masuk atau folder spam email Anda untuk melakukan verifikasi akun.");
              closeAuthModal();
              // Mengeluarkan sesi user sementara agar tidak menggantung status login sebelum verifikasi dilakukan
              auth.signOut();
            })
            .catch(err => {
              console.error("Gagal mengirim email verifikasi:", err);
              showAlert("Akun berhasil dibuat, tetapi gagal mengirim email verifikasi.");
            });
        })
        .catch(err => {
          showAlert(`Gagal Daftar: ${escapeHTML(err.message)}`);
        });
    }
  } else {
...
function renderQuizQuestion() {
  ...
  document.getElementById('quiz-category').innerText = escapeHTML(q.category);
  document.getElementById('current-question-num').innerText = currentQuizIndex + 1;
  document.getElementById('quiz-question-text').innerText = q.question; // Menggunakan innerText untuk proteksi XSS bawaan browser
  ...
  q.options.forEach((opt, index) => {
    ...
    btn.innerHTML = `
      ...
      <span class="text-sm mt-0.5 leading-relaxed">${escapeHTML(opt)}</span>
    `;
  ...
  quizQuestions.forEach((q, idx) => {
    ...
    expDiv.innerHTML = `
      <div class="flex items-center justify-between">
        <span class="text-xs font-bold text-gold">${escapeHTML(q.category)} - Soal ${idx + 1}</span>
      ...
      <div class="bg-stone-900 p-3.5 rounded-xl border border-stone-800 text-xs text-stone-300">
        <span class="block text-gold font-bold mb-1">Pilihan Terbaik: ${letters[q.answer]}. ${escapeHTML(q.options[q.answer])}</span>
        <span class="block text-stone-400 mt-2"><strong>Pembahasan:</strong> ${escapeHTML(q.explanation)}</span>
      </div>
    `;
...
function saveUserQuizScore(uid, score, correct, accuracy) {
  if (typeof db !== 'undefined' && db !== null) {
    // Validasi tipe data sebelum menyimpan ke database
    const sanitizedUid = escapeHTML(uid);
    db.collection('artifacts').doc(appId).collection('users').doc(sanitizedUid).collection('quiz_history').add({
      score: Number(score),
...
function loadUserData(uid) {
  if (typeof db !== 'undefined' && db !== null) {
    const sanitizedUid = escapeHTML(uid);
    db.collection('artifacts').doc(appId).collection('users').doc(sanitizedUid).collection('quiz_history')
      .orderBy('timestamp', 'desc').limit(1).get()
      .then(snapshot => {
        if (!snapshot.empty) {
          const data = snapshot.docs[0].data();
          showAlert(`Halo! Skor latihan terakhir Anda di database kami adalah ${Number(data.score)} (${Number(data.correctCount)} Soal Benar)`);
        }
      }).catch((e) => console.log("Belum ada riwayat / Firebase index dibatasi:", e));
  }
}

// 105-Item Simulator Controllers with Input Hardening
function handleTryoutPurchase() {
  ...
  // Validasi Klien Lapangan Checkout Form
  const payNameInput = document.getElementById('pay-name').value.trim();
  const payEmailInput = document.getElementById('pay-email').value.trim();
  
  const nameRegex = /^[A-Za-z\s']{3,50}$/;
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

  if (!nameRegex.test(payNameInput)) {
    showAlert("Gagal: Nama lengkap hanya boleh berupa huruf, spasi, dan minimal 3 karakter.");
    return;
  }

  if (!emailRegex.test(payEmailInput)) {
    showAlert("Gagal: Silakan masukkan format email yang valid.");
    return;
  }

  const method = document.getElementById('pay-method').value;
  document.getElementById('pay-method-display').innerText = method === 'QRIS' ? 'QRIS' : 'Transfer Bank';
...
function simulatedPaymentSuccess() {
  ...
  if (currentUser && typeof db !== 'undefined' && db !== null) {
    ...
    db.collection('artifacts').doc(appId).collection('users').doc(currentUser.uid).collection('transactions').add({
      ...
      price: 5000,
      buyerName: escapeHTML(payNameInput),
      buyerEmail: escapeHTML(payEmailInput),
      status: "SUCCESS",
      timestamp: firebase.firestore.FieldValue.serverTimestamp()
...
function renderSimulatorQuestion() {
  ...
  document.getElementById('sim-q-cat').innerText = escapeHTML(q.category);
  document.getElementById('sim-q-text').innerText = q.question; // Menggunakan innerText untuk memblokir HTML injection

  const figContainer = document.getElementById('sim-figural-container');
  // Proteksi: Gambar spasial (SVG) hanya boleh dijalankan jika bersumber dari config lokal tepercaya (statis)
  if (q.isFigural && q.figuralSvg) {
    figContainer.innerHTML = q.figuralSvg; // Aman karena terdefinisi statis dan tidak menerima masukan bebas pengguna
  ...
  if (typeof auth !== 'undefined' && auth !== null) {
    try {
      auth.onAuthStateChanged((user) => {
        if (user) {
          // Hanya izinkan masuk jika email sudah diverifikasi secara resmi
          if (user.emailVerified) {
            currentUser = user;
            ...
            // Autentikasi ditolak jika belum diverifikasi, paksa logout secara aman
            currentUser = null;
            auth.signOut();
          }
        } else {
